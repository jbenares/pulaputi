<x-app-layout>
       ...
       <br><br><br>
       <center>
    <a href="{{ route('wallet_transfer') }}" class="py-2 px-4  bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ">
         Go Back
    </a>
</center><br>
            ...
            <div class="w-full h-full ">
                ...
                <p class="absolute text-sm italic text-gray-800 dark:text-white top-2 right-2">
                    ...
                    <p class="mt-4 text-lg text-green-700 dark:text-green text-center">
                        Wallet Successfully Transfered!
                    </p>
                   @foreach($details AS $d)
                    <p class="px-6 py-2 text-sm flex font-thin text-gray-700 dark:text-gray-50">
                       Transfer Date: {{ $d->transaction_date }}<br>
                       Transfer To: {{ $d->ref_code }}<br>
                       Transfer Amount: {{ number_format($d->transfer_amount,2) }} <br>
