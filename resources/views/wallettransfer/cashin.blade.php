<x-app-layout>
    ...
    @if (session('status'))
        ...
        <form action="{{ route('cashin_process') }}" method="POST">
        @csrf
       
        <div class="flex flex-col mb-2">
                    <div class=" relative ">
                        <select name="method" required id="method"  class=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" />
                            <option value="">Cashin Method</option>
                    ...
                    <div class="flex flex-col mb-2">
                    <div class=" relative ">
                        <input type="text" id="source" name="source" value="{{old('source')}}" class=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Source"/>
                        </div>
                       <span id='refcode_message' class='text-xs'></span>
                    </div>
                    <div class="flex flex-col mb-2">
                    <div class=" relative ">
                        <input name="amount" required type="text" id="amount" value="{{old('amount')}}" class=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Amount to Cashin"/>
                        </div>
